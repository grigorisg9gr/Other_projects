...
for i = 1:batchsize:numneg
  ...
  parfor k = 1:thisbatchsize 
    ...
    for mm=1:size(neg(j).pos_boxes,1)                                   % added this fix, to ignore the area of the respective positive bbox
        xx_a=neg(j).pos_boxes(mm,1):neg(j).pos_boxes(mm,3);
        for yy=neg(j).pos_boxes(mm,2):neg(j).pos_boxes(mm,4)
            im(yy,xx_a)=0;
        end
    end
...
for i = 1:numneg
  ...
  if size(feat,2) > rsize(2) && size(feat,1) > rsize(1)
    for j = 1:rndneg
      fl=false;                                             % added this fix, to find a random area, not overlapping with the positive bbox(es)
      while ~fl
        x = random('unid', size(feat,2)-rsize(2)+1);
        y = random('unid', size(feat,1)-rsize(1)+1);
        cnt=0;                                              
        for mm=1:size(neg(i).pos_boxes,1)
            box = intersectionb(neg(i).pos_boxes(mm,:),[x,y,x+rsize(2)-1,y+rsize(1)-1]);
            if  ((box(2)-box(1))*(box(4)-box(3))<=0)||((box(2)-box(1))<=0)  
                cnt=cnt+1;
            end
        end
        if cnt==size(neg(i).pos_boxes,1)
            fl=true;
        end
      end
...
function inters_box = intersectionb(box1,box2)
% intersection of the two boxes box1 and box2
mini = min(box1(:,[3 4]), box2(:,[3 4]));
maxi = max(box1(:,[1 2]), box2(:,[1 2]));
inters_box = [maxi(:,1) maxi(:,2) mini(:,1) mini(:,2)];
